...
**Latest artifact snapshot.** The clean training table, taxonomy table, and sample submission have no duplicate rows or duplicate train keys. The soundscape label file is different: 1,478 rows collapse to 739 unique rows, so all soundscape prevalence analysis below uses the deduplicated view.
"""
            ),
...
"""
            ...
            md(
                """
**Latest artifact snapshot.** The run contains 35,549 recordings across 206 primary labels. The median class has 125 recordings, the largest class has 499, and the smallest class has 1. The top 10 classes represent 13.9% of recordings, while the top 30 already cover 40.3%, so local validation should always include per-class diagnostics.
"""
            ),
...
"""
            ...
            md(
                """
**Latest artifact snapshot.** Secondary labels contain 7,431 total mentions across 161 labels. The most common labels (`grekis`, `whtdov`, `undtin1`, `yecpar`, and `rufhor2`) are frequent enough to be useful as soft-label or co-occurrence signals, but they should not replace the primary-label objective in the first baseline.
"""
            ),
...
**Latest artifact snapshot.** Rating `0.0` appears in 12,849 rows, while ratings `4.0` and `5.0` together account for 14,863 rows. Collection is split between XC (23,043 rows) and iNat (12,506 rows). Only 847 recordings, or 2.38%, fall inside the rough Pantanal box used here, even though those rows cover 119 species.
"""
            ),
...
**Latest artifact snapshot.** After deduplication, there are 739 unique soundscape segments. They are genuinely multi-label: the median segment has 4 labels, the 90th percentile has 7, and the maximum has 10. Labeled segments cluster most strongly around evening/night hours, especially 20:00-23:00.
"""
            ),
...
- Treat the soundscape annotations as a domain-specific calibration set after deduplication, not as clean clip-level training rows.
"""
            ),
